...
Generation Generation::formNextGeneration(double crossover, double mutationchance, double severity, int tourneysize, int gen, double mjost)
{
   ...
   for (int indb = 10;indb < 20;indb++)
   {
      ...
      g.dnavect[indb].mutate(mutationchance, severity, reactionlock, mjost);
   }
   ...
   for (int indc = 20;indc < 30;indc++)
   {
      ...
      g.dnavect[indc].mutate(mutationchance, severity * 2, reactionlock, mjost);
   }
...
void Generation::simulate(int num)
{
   ...
   KeyState ks;
   //Random ran;
   GameSpace gs(1, 0, 800, 0, 600, seed, true, "test.lua", "shinku.lua");

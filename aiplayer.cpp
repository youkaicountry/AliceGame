...
KeyState AIPlayer::update(GameSpace* gs, Player* p, DNA dna, KeyState kes)
{
    ...
    list<NeoBullet>::iterator bu;
    for(bu=gs->lEnemyBullets.begin(); bu != gs->lEnemyBullets.end();bu++)
    {
       TwoVector vb(bu->x - p->getX(), bu->y - p->getY());
       double dtest = vb.getLength(); dtest -= p->getRadius();dtest -= bu->radius;
    ...
    {
       TwoVector v(i->x - p->getX(), i->y - p->getY());
